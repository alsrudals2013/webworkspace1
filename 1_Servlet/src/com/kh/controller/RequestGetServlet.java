...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  request의 parameter 영역안에 존재=> key-value세트로 담겨있다(name속성값-value속성값)
		 *  
		 ...
		 *  */
		String name = request.getParameter("name"); // "민경민", ""(텍스트 상자가 빈경우 빈 문장열이 넘어간다)
		...
		String[] foods = request.getParameterValues("food"); // ["한식","일식"] / null (체크한게 없다면 null이반환)
		
		/*
		 * 이 뽑아낸 값들을 가지고 요청 처리해줘야함(db와 상호작용을함)
		 * 보통의 흐름 : Service의 메소드 호출 시 뽑은 값들을 전달(Wrapping) - DAO 호출 - DB SQL문 결과값 반환
		 * 
		 * int result = new MemberService().inserMember(m);
		 * if(result >0) : 성공이라고보고 commit, 실패시 rollback(Service가 처리)
		 * Service와 Dao단은 거의 변동없이 View와 Controller단의 소스코드상의 변화가 많을것이다.
		 * 
		 * 위 요청처리를 다 했따는 가정하에 사용자가 보게될 응답페이지를 꾸밀예정.
		 * 
		 * 장점 : Java코드내에 작성하기 때문에 반복문,조건문등 유용항 메소드 활용이가능
		 * 
		 * 단점 : 복잡, 혹시라도 추후 html을 수정하고자한다면 Java소스코드 내에서 수정이 이루어지기때문에
